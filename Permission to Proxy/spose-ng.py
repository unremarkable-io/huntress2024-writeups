...
class Spose(object):
    def __init__(self):
        req = URLRequest()
        parser = argparse.ArgumentParser(prog='Spose by Petruknisme', description='Squid Pivoting Open Port Scanner')
        ...
        proxy = results.proxy
        common_ports = [1, 3, 4, 6, 7, 9, 13, 17, 19, 20, 21, 22, 23, 24, 25, 26, 30, 32, 33, 37, 42, 43, 49, 53, 70,
        ...
        for port in tqdm(sorted(common_ports)):
            try:
                url = f'http://{target}:{port}'
                if DEBUG:
                    print(f'Trying URL "{url}": ', end='')
                data = req.get(url, "{}".format(proxy))
                code = data.getcode()
                if code != 403:
                    print(f'{target}:{port} (code:{code}) is NOT 403!')
